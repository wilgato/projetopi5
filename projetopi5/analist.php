...
$ret1=mysqli_query($con,"select * from tblpaciente where registro='$fmid'");
...
?>            
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Monitoramento do Paciente: <?php  echo  utf8_encode($row1['name']);?> </h1>
                        <hr />
                        <?php } ?>
...
$query = "SELECT * FROM tblmedicao where paciente_id='$fmid' ORDER BY id_medicao DESC LIMIT 20 ";
$runQuery = mysqli_query($conn, $query);

while ($row = mysqli_fetch_array($runQuery)) {
    // Add data to arrays
    $dataTemperature[] = $row['temp_amb'];
    $dataHumidity[] = $row['um_amb'];
    $dataPressure[] = $row['pres_amb'];
...
?>             
                <div class="w-full md:w-1/2 xl:w-1/3 p-6">
                    ...
                    <div class="bg-white border-transparent rounded-lg shadow-xl">
                        <div class="bg-gradient-to-b from-gray-300 to-gray-100 uppercase text-gray-800 border-b-2 border-gray-300 rounded-tl-lg rounded-tr-lg p-2">
                            <h5 class="font-bold uppercase text-gray-600"><?php  echo utf8_encode("Temp. Corporal")?></h5>
                        </div>
                        <div class="p-5">
...
$query = "SELECT * FROM tblpaciente where registro= '$fmid' ORDER BY id DESC LIMIT 1";
...
if ($resultado->num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $queryBPM = "SELECT * FROM tblmedicao where paciente_id= '$registro' ORDER BY id_medicao DESC LIMIT 1";
       ...
       if ($sexo == "Masculino") {
                ...
                if ($idade >= 18 && $idade <= 25) {
                    ...
                    if ($bpm >= 60 && $bpm <= 64) {
                        $status_bpm = "Excelente";
                        $cor_fonte = "green"; // Define a cor da fonte para verde 
                    } elseif ($bpm >= 65 && $bpm <= 72) {
                        $status_bpm = "Boa";
                        $cor_fonte = "green"; // Define a cor da fonte para verde 
                    } elseif ($bpm >= 73 && $bpm <= 76) {
                        $status_bpm = "Normal";
                        $cor_fonte = "green"; // Define a cor da fonte para verde 
                    } elseif ($bpm >= 77 && $bpm <= 82) {
                        $status_bpm = "Menos boa";
                        $cor_fonte = "orange"; // Define a cor da fonte para laranja
                    } else {
                        $status_bpm = "Ruim";
                    ...
                    if ($bpm >= 61 && $bpm <= 65) {
                        ...
                        $status_bpm = "Boa";
                        $cor_fonte = "green"; // Define a cor da fonte para verde 
                    } elseif ($bpm >= 74 && $bpm <= 77) {
                        $status_bpm = "Normal";
                        $cor_fonte = "green"; // Define a cor da fonte para verde 
                    } elseif ($bpm >= 78 && $bpm <= 83) {
                        $status_bpm = "Menos boa";
                        $cor_fonte = "orange"; // Define a cor da fonte para laranja
                    } else {
                        $status_bpm = "Ruim";
  ...
  <div class="fade-out"></div>
         ...
         <?php 
            ...
            echo '<div class="w-full md:w-1/2 xl:w-1/3 p-6">';
        echo '<!--Graph Card-->';
        echo '<div class="bg-white border-transparent rounded-lg shadow-xl">';
        echo '<div class="bg-gradient-to-b from-gray-300 to-gray-100 uppercase text-gray-800 border-b-2 border-gray-300 rounded-tl-lg rounded-tr-lg p-2">';
        echo '<h5 class="font-bold uppercase text-gray-600">'  . utf8_encode('Info do Paciente') . ' </h5>';
        echo '</div>';
        echo '<div class="p-5">';
        echo '<table class="w-full p-5 text-gray-700">';
        echo '<thead>';
        echo '<h4 style="color: red;">' . utf8_encode('Medição não Encontrada') . '</h4>';
        echo '<tr>';
        echo '<th class="text-left text-blue-90">Nome</th>';
        echo '</tr>';
        echo '<tr>';
        echo '<td>' . utf8_encode($pacienteNome) . '</td>';
        echo '</tr>';
        echo '<table class="w-full p-5 text-gray-700">';
        echo '<tr>';
        echo '<th class="text-left text-blue-900">Idade</th>';
        echo '<th class="text-left text-blue-90">Sexo</th>';
        echo '</tr>';
        echo '<tr>';
        echo '<td>' . $idade . '</td>';
        echo '<td>' . $sexo . '</td>';
...
$query = "SELECT * FROM tblmedicao where paciente_id='$fmid' ORDER BY postingTime DESC LIMIT 20 ";
$runQuery = mysqli_query($conn, $query);
